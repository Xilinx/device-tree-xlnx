	...
	#scaler
		...
		if {[llength $connected_in_ip] != 0} {
			...
			if {[string match -nocase $connected_in_ip_type "v_proc_ss"]|| [string match -nocase $connected_in_ip_type "v_tpg"] || [string match -nocase $connected_in_ip_type "v_smpte_uhdsdi_rx_ss"]|| [string match -nocase $ip_type "mipi_csi2_rx_subsystem"]} {
				...
				hsi::utils::add_new_dts_param "$scaler_port_node" "xlnx,video-format" 3 int
			...
			if {[string match -nocase $connected_in_ip_type "v_hdmi_rx_ss"]} {
				...
				hsi::utils::add_new_dts_param "$hdmi_port_node" "xlnx,video-format" 3 int
				...
				if {[string match -nocase $fifo_connected_out_ip_type "axis_register_slice"]} {
					...
					if {[llength $axis_reg_slice_ip]} {
						...
						if {[string match -nocase $axis_reg_slice_connected_out_ip_type "v_frmbuf_wr"]} {
							...
							hsi::utils::add_new_dts_param "$hdmi_port1_node" "xlnx,video-format" 3 int
						...
						if {[string match -nocase $axis_reg_slice_connected_out_ip_type "v_scenechange"]} {
							...
							hsi::utils::add_new_dts_param "$scd_port1_node" "xlnx,video-format" 3 int
						...
						dtg_warning "connected stream of axis_reg_slice_ip is NULL...check the design"
					}
				}
			}
		...
		foreach connect_ip $connected_in_ip {
			...
			if {[llength $connect_ip]} {
				...
				if {[string match -nocase $connected_in_ip_type "v_hdmi_rx_ss"]} {
					...
					hsi::utils::add_new_dts_param "$port_node" "xlnx,video-format" 3 int
					hsi::utils::add_new_dts_param "$port_node" "xlnx,video-width" $max_data_width int
					set hdmirx_node [add_or_get_dt_node -n "endpoint" -l csc_in -p $port_node]
					hsi::utils::add_new_dts_param "$hdmirx_node" "remote-endpoint" hdmi_rx_out reference
				}
			} else {
				dtg_warning "$drv_handle: input port pin S_AXIS is not connected"
			}
				...
				if {[string match -nocase $connected_out_ip_type "v_proc_ss"]|| [string match -nocase $connected_out_ip_type "v_frmbuf_wr"]} {
					if {[llength $ports_node]} {
						...
						if {[string match -nocase $connected_out_ip_type "v_frmbuf_wr"]} {
							...
							hsi::utils::set_os_parameter_value "vcap_sdirx_count" $vcap_sdirx_count
							hsi::utils::add_new_dts_param $vcap_sdirx "compatible" "xlnx,video" string
							hsi::utils::add_new_dts_param $vcap_sdirx "dmas" "$connect_out_ip 0" reference
